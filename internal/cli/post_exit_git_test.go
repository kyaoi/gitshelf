...
func TestRunPostExitGitActionCommitsNestedConfiguredStoragePaths(t *testing.T) {
	root := t.TempDir()
	if _, err := runGitCommand(root, "init"); err != nil {
		t.Fatalf("git init failed: %v", err)
	}
	...
	if err := runPostExitGitAction(root, postExitGitSettings{
		Action:        postExitGitCommit,
		CommitMessage: "test: commit nested configured storage",
	}); err != nil {
		t.Fatalf("runPostExitGitAction failed: %v", err)
	}
	...
	for _, want := range []string{".shelf/config.toml", "data/shelf-store/tasks"} {
		if !strings.Contains(files, want) {
			t.Fatalf("expected committed paths to include %q, got %q", want, files)
		}
	}
...
func TestRunPostExitGitActionCommitsStatusOnlyChanges(t *testing.T) {
	root := t.TempDir()
	if _, err := runGitCommand(root, "init"); err != nil {
		t.Fatalf("git init failed: %v", err)
	}
	...
	if _, err := shelf.SetTask(root, task.ID, shelf.SetTaskInput{Status: &done}); err != nil {
		t.Fatalf("set task status failed: %v", err)
	}
	if err := runPostExitGitAction(root, settings); err != nil {
		t.Fatalf("second runPostExitGitAction failed: %v", err)
	}
	...
	if strings.TrimSpace(count) != "2" {
		t.Fatalf("expected two commits, got %q", count)
	}
}

func TestRunPostExitGitActionCommitsStorageRootMoves(t *testing.T) {
	root := t.TempDir()
	if _, err := runGitCommand(root, "init"); err != nil {
		t.Fatalf("git init failed: %v", err)
	}
	...
	oldPath := filepath.Join(root, ".shelf", "tasks", task.ID+".md")
	newPath := filepath.Join(root, "shelf", "tasks", task.ID+".md")
	if err := os.MkdirAll(filepath.Dir(newPath), 0o755); err != nil {
		t.Fatalf("mkdir new tasks dir failed: %v", err)
	}
	if err := os.Rename(oldPath, newPath); err != nil {
		t.Fatalf("move task file failed: %v", err)
	}

	if err := runPostExitGitAction(root, postExitGitSettings{
		Action:        postExitGitCommit,
		CommitMessage: "test: move storage root",
	}); err != nil {
		t.Fatalf("second runPostExitGitAction failed: %v", err)
	}

	files, err := runGitCommand(root, "show", "--pretty=", "--name-status", "HEAD")
	if err != nil {
		t.Fatalf("git show failed: %v", err)
	}
	for _, want := range []string{"M\t.shelf/config.toml", "R100\t.shelf/tasks/" + task.ID + ".md\tshelf/tasks/" + task.ID + ".md"} {
		if !strings.Contains(files, want) {
			t.Fatalf("expected committed paths to include %q, got %q", want, files)
		}
	}
